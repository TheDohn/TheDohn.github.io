...
- For the values present in the training data, the EXT_SOURCE_1 feature had a high amount of correlation with the TARGET, but unfortunately was missing many values. Rather that simply impute missing values with the median I imputed based a linear fit to DAYS_BIRTH for each gender. 
